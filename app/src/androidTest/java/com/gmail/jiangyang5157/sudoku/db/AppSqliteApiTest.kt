...
class AppSqliteApiTest {
    ...
    fun setUp() {
        ...
        assertNotEquals(-1, AppSqliteApi.getInstance(appContext).insertPuzzle("", "", "", "11", "11"))
        assertNotEquals(-1, AppSqliteApi.getInstance(appContext).insertPuzzle("", "", "", "22", "22"))
        assertNotEquals(-1, AppSqliteApi.getInstance(appContext).insertPuzzle("", "", "", "33", "2222"))
        assertNotEquals(-1, AppSqliteApi.getInstance(appContext).insertPuzzle("", "", "", "44", "11"))
        assertNotEquals(-1, AppSqliteApi.getInstance(appContext).insertPuzzle("", "", "", "55", "2222"))
    ...
    fun test_queryLikePuzzles() {
        val appContext = InstrumentationRegistry.getTargetContext()
        val cursor = AppSqliteApi.getInstance(appContext).queryLikePuzzles(PuzzleTable.Column.KEY_BEST_TIME, "22", BaseSqliteApi.OrderBy.asc(PuzzleTable.Column.KEY_DATE))
